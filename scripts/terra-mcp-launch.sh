...
while IFS= read -r _line || [ -n "$_line" ]; do
  ...
  case "$_k" in
    ...
    MCP_TERRA_BUDGET_USD|MCP_TERRA_BUDGET_WINDOW_DAYS|\
    MCP_TERRA_MAX_SUBMITS_PER_SESSION|MCP_TERRA_CONTROLLED_ACCESS|\
    MCP_TERRA_SLACK_WEBHOOK|MCP_TERRA_SLACK_BOT_TOKEN|MCP_TERRA_SLACK_CHANNEL|\
    MCP_TERRA_SMTP_HOST|MCP_TERRA_SMTP_PORT|MCP_TERRA_SMTP_USER|MCP_TERRA_SMTP_PASS|\
    MCP_TERRA_SMTP_FROM|MCP_TERRA_SMTP_RELAY|MCP_TERRA_SMTP_STARTTLS|\
    MCP_TERRA_EMAIL_RECIPIENT_OVERRIDE|PATH|HOME)
      export "$_k=$_v" ;;
    *)
