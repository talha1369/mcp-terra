...
allowed-tools: mcp-terra:terra_health, mcp-terra:terra_get_workspace, mcp-terra:terra_list_method_configs, mcp-terra:terra_register_method, mcp-terra:terra_create_method_config, mcp-terra:terra_submit_workflow, mcp-terra:terra_get_submission, mcp-terra:terra_summarize_submissions, mcp-terra:terra_get_workflow_outputs, mcp-terra:terra_get_workflow_logs, mcp-terra:terra_classify_workflow_failure, mcp-terra:terra_render_audio_summary, mcp-terra:terra_send_run_report_email, Read, Edit, Bash, Task
---
...
- **`Failed`** → read the workflow's failure messages, then pass them to
  `terra_classify_workflow_failure(failure_text=<message>)` for a **deterministic
  category** (the WDL analogue of the notebook bug-triager — cheap, no LLM read).
  Cromwell/Google-Batch errors fall into a stable set of categories — fix by the
  returned `category`:
