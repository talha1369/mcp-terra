...
**318/318 tests pass** across 47 attack classes. The table below is generated
...
| CC-ControlledAccess3 | 7 | Round-4 egress closure + retry safety: **a structural meta-test asserting every data-returning tool carries a controlled-access check**, `method_config` projected to key names, `submission` projected to ids+statuses, `workflow_logs` refuses a stderr path outside the queried workspace bucket, `list_bucket`/`batch`/audio source-enforced (Cloud-TTS→local `say`), retry aborts on the kill-switch hook + total-budget bound, the `terra://health` resource is minimal + data-free |
