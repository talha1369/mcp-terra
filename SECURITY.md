...
**336/336 tests pass** across 48 attack classes. The table below is generated
...
| CC-ControlledAccess3 | 22 | Round-4 egress closure + retry safety: a **fail-closed AST meta-test** (every registered tool must be explicitly classified; every data tool must carry a *runtime* guard call — a docstring mention can't satisfy it), `method_config` projected to **counts** (values AND key names withheld; sentinel-identifier test), `submission` projected to ids+statuses, `workflow_logs` refuses a stderr path outside the queried workspace bucket + flags per-task truncation, audio `say` fed via **stdin not argv**, retry aborts on the kill-switch hook (interruptible backoff) + deadline-capped, the `terra://health` resource is minimal + data-free |
