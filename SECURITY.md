...
| Adversarial security review (boot-chain + lease-after-exit follow-ups) | **CRITICAL**: the sha-pin covered only the second-stage runner — the FIRST-stage `start_runner.sh` is itself fetched from the co-member-writable bucket and runs before any check. **HIGH**: `LEASE_ABORTED` was set only DURING the watch loop, so `LOST_CLAIM`/`RUNNER_ABORT` arriving after papermill exits but before the terminal write was missed. **MED**: a race between launching papermill and recording its PGID could orphan it on a halt. | Fixed/mitigated — the second-stage runner is now fetched from a **generation-pinned (immutable) URI** (gsutil) on top of its sha-verify; the lease is **re-checked after `wait` AND immediately before the durable terminal write** (no result/spec write if the claim was lost); the PGID is recorded **synchronously** by the setsid'd child (`echo $$`) before the workload execs. The first-stage start-script residual is a **documented co-member-trust-boundary limitation** (acknowledged limitations §7) — content-addressed + create-time sha-verified, with the second stage immutable+signed; full closure needs Leonardo-side immutable bootstrap. +4 regression tests (CC-Hardening). |
...
7. **First-stage VM start script is fetched from the workspace bucket.** Leonardo runs `start_runner.sh` from the workspace bucket on every VM boot/resume, and it receives the runner secret via Leonardo `customEnvironmentVariables`. That bucket is writable by **workspace co-members**, so a *malicious* co-member (someone you have granted workspace write access) could overwrite the first-stage script between runtime creation and the next boot and run code with the runner secret. This sits within the **co-member trust boundary** — a co-member already has full read access to your workspace data and write access to its bucket — analogous to the compromised-host limitation. Mitigations in place: the script is content-addressed and **sha256-verified at create time**; the **second-stage runner it launches is sha256-verified on the VM AND fetched from a generation-pinned (immutable) URI**; and every job spec is independently **HMAC-signed**. Be precise about the residual, though: a swapped first-stage script runs **with the runner secret** (Leonardo delivers it via `customEnvironmentVariables`), so a co-member who controls that script **does** obtain the key and can then sign accepted jobs — the HMAC boundary alone does not stop a co-member who can overwrite the first stage. **Mitigation (recommended):** run in a **single-user workspace** (no co-members ⇒ no exposure), or restrict who can write the workspace bucket (object/prefix IAM, or a dedicated bucket) so the first-stage script cannot be overwritten. We **verified** that the obvious code fix — generation-pinning the `startUserScriptUri` (`gs://…#<gen>`) — is **not viable**: Leonardo validates the script URI at create time via the `google-cloud-storage` `BlobId.of(bucket, name)` API (which has no generation parameter), so a `#<generation>` suffix is treated as a *literal object name* and the create call fails with object-not-found (traced in the Leonardo source: `RuntimeServiceInterp.validateBucketObjectUri` → `GoogleStorageInterpreter` `BlobId.of(bucket, name)`). Full closure therefore requires a **Leonardo-side immutable/inline bootstrap** (an upstream feature request), not an MCP change.
...
**440/440 tests pass** across 50+ attack classes. The table below summarizes the
